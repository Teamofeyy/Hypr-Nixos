...
let
  throneCli = pkgs.writeShellScriptBin "throne" "exec ${pkgs.throne}/bin/Throne \"$@\"";
  bitwardenWeb = pkgs.writeShellApplication {
    name = "bitwarden";
    runtimeInputs = [ pkgs.chromium ];
    text = ''
      exec chromium --new-window --app=https://vault.bitwarden.com --ozone-platform=wayland "$@"
    '';
  };
...
in
{
  ...
  systemd.user.services.polkit-agent =
    let
      ...
      matePolkitAgent = "${pkgs.mate-polkit}/libexec/polkit-mate-authentication-agent-1";
      polkitAgentScript = pkgs.writeShellScript "polkit-agent" ''
        if ${pkgs.procps}/bin/pgrep -u "$UID" -f '/hyprpolkitagent($| )' >/dev/null 2>&1; then
          exit 0
        fi
        if [ -x "${hyprpolkitAgent}" ]; then
          exec "${hyprpolkitAgent}"
        fi
        if [ -x "${matePolkitAgent}" ]; then
          exec "${matePolkitAgent}"
        fi
